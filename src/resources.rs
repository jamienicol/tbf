    ...
    R: gfx::Resources
{
        ...
        where F: gfx::Factory<R>
    {
        ...
        full_path.push(&path);
        let img = image::open(full_path).expect(
            &format!("Error opening image file {}", &path)
        ).to_rgba();
        let (w, h) = img.dimensions();
        let kind = gfx::texture::Kind::D2(w as u16, h as u16, gfx::texture::AaMode::Single);
        let (_, view) = factory.create_texture_immutable_u8::<Rgba8>(kind, Mipmap::Provided, &[&img])
